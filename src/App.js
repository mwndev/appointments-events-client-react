...
import { PhoneHeader, Header } from "./Header";
...
const Page = styled.div`
  ...
  @media only screen and (max-width: 800px) {
    margin-top: ${(p) => p.theme.pHeaderHeight};
  }
`;
...
function App() {
  ...
  const [fnToConfirm, setFnToConfirm] = useState(
    () => () => console.log("fntoconfirm")
  );
  ...
  useEffect(() => {
    ...
    windowAlert(
      "This website is still a work in progress. What's currently still missing is the mobile layout."
    );
  ...
  return (
    <ThemeProvider theme={theme1}>
      <AppointmentContext.Provider
        value={{ fullyFilteredAppointmentes, setFullyFilteredAppointments }}
      >
        <WindowAlertContext.Provider
          value={{ activeA, setActiveA, messageA, windowAlert }}
        >
          <WindowConfirmContext.Provider
            value={{ active, setActive, message, fnToConfirm, windowConfirm }}
          >
                ...
                <Router>
                  ...
                  <Page>
                    <Routes>
                      ...
                      <Route
                        path={"/reserve/:tokenid"}
                        element={<Reservation />}
                      />
                      <Route
                        path={"/newpassword/:tokenid"}
                        element={<NewPassword />}
                        key={"123213"}
                      />
                      <Route
